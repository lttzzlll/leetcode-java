...
//        int minDistance = Integer.MAX_VALUE, val = Integer.MIN_VALUE;
...
//    }
    ...
    public int threeSumClosest(int[] nums, int target) {
        Arrays.sort(nums);
        int res = nums[0] + nums[1] + nums[2]; // default value
        for (int i = 0; i < nums.length - 2; i++) {
            int j = i + 1;
            int k = nums.length - 1;
            while (j < k) {
